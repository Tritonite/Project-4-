...
public class MapData
{
    ...
    private void parseParamHeader(String inParamStr) throws IOException
    {
        ...
        for (int i = 0; i < split.length; i++)
        {
            if (split[i].equalsIgnoreCase(inParamStr))
            {
                ...
                else if (inParamStr.equalsIgnoreCase(WSPD))
                {
                    paramPositions.put(WSPD, i);
                }
                else if (inParamStr.equalsIgnoreCase(PRES))
                {
    ...
    public void parseFile() throws IOException
    {
        ...
        while(strg != null)
        {
            ...
            wspdData.add(
                    new Observation(Double.parseDouble(info[getIndexOfString(WSPD)]), info[getIndexOfString(STID)]));
            presData.add(
                    new Observation(Double.parseDouble(info[getIndexOfString(PRES)]), info[getIndexOfString(STID)]));
    ...
    private void calculateAllStatistics()
    {
        ...
        {
            // checks if data is valid then the value is stored
            if (wspdData.get(i).isValid() && wspdData.get(i).getValue() > maxData)
            {

                maxData = wspdData.get(i).getValue();
                countMax = i;
            }
        }
        ...
        {
            // checks if data is valid, the value is stored
            if (wspdData.get(i).isValid() && wspdData.get(i).getValue() < minData)
            {

                minData = wspdData.get(i).getValue();
                countMin = i;
        ...
        for (int i = 0; i < numberOfStations; i++)
        {
            ...
            if (wspdData.get(i).isValid())
            {
                dataTotal += wspdData.get(i).getValue();
                countAvg++;
            }
        ...
        if (loss < NUMBER_OF_MISSING_OBSERVATIONS)
        {
            ...
            dataMax.put(WSPD, new Statistics(maxData, wspdData.get(countMax).getStid(), utcDateTime,
                    (numberOfStations - loss), StatsType.MAXIMU));
            dataMin.put(WSPD, new Statistics(minData, wspdData.get(countMin).getStid(), utcDateTime,
                    (numberOfStations - loss), StatsType.MINIMUM));
            dataAvg.put(WSPD,
                    new Statistics(averageData, MESONET, utcDateTime, (numberOfStations - loss), StatsType.AVERAGE));
        }
        ...
        {
            // checks if data is valid then the value is stored
            if (presData.get(i).isValid() && presData.get(i).getValue() > maxData)
            {

                maxData = presData.get(i).getValue();
                countMax = i;
            }
        }
        ...
        {
            // checks if data is valid, the value is stored
            if (presData.get(i).isValid() && presData.get(i).getValue() < minData)
            {

                minData = presData.get(i).getValue();
                countMin = i;
        ...
        for (int i = 0; i < numberOfStations; i++)
        {
            ...
            if (presData.get(i).isValid())
            {
                dataTotal += presData.get(i).getValue();
                count3++;
            }
        ...
        if (loss < NUMBER_OF_MISSING_OBSERVATIONS)
        {
            dataMax.put(PRES, new Statistics(maxData, presData.get(countMax).getStid(), utcDateTime,
                    (numberOfStations - loss), StatsType.MAXIMU));
            dataMin.put(PRES, new Statistics(minData, presData.get(countMin).getStid(), utcDateTime,
                    (numberOfStations - loss), StatsType.MINIMUM));
            dataAvg.put(PRES,
                    new Statistics(averageData, MESONET, utcDateTime, (numberOfStations - loss), StatsType.AVERAGE));
        }
